...
void SAM::InsertBreath(unsigned char mem59) {
  ...
  while((index = phonemeindex[pos]) != END) {
    ...
    if (len < 232) {
      if (index == BREAK) {
      } else if (!(sam_flags[index]& FLAG_PUNCT)) {
...
void SAM::Code41240() {
  ...
  while(phonemeindex[pos] != END) {
    ...
    if ((sam_flags[index]& FLAG_STOPCONS)) {
      if ((sam_flags[index]& FLAG_PLOSIVE)) {
        unsigned char A;
        unsigned char X = pos;
        while(!(A = phonemeindex[++X])); /* Skip pause */

        if (A != END) {
          if ((sam_flags[A] & 8) || (A == 36) || (A == 37)) {++pos; continue;} // '/H' '/X'
        }
...
void SAM::rule_alveolar_uw(unsigned char X) {
  // ALVEOLAR flag set?
  if (sam_flags[phonemeindex[X-1]] & FLAG_ALVEOLAR) {
...
void SAM::rule_g(unsigned char pos) {
  ...
  if ((index != 255) && ((sam_flags[index] & FLAG_DIP_YX) == 0)) {
...
void SAM::Parser2() {
  ...
  while((p = phonemeindex[pos]) != END) {
    ...
    if ((pf & FLAG_DIPTHONG)) rule_dipthong(p, pf, pos, mem59);
      ...
      if (!phonemeindex[pos+1]) { // If following phoneme is a pause, get next
        ...
        if (p!=END && (sam_flags[p] & FLAG_VOWEL) && stress[pos+2]) {
      ...
      else if (sam_flags[prior] & FLAG_VOWEL) change(pos, 18, "<VOWEL> R -> <VOWEL> RX"); // Example: ART
    } else if (p == 24 && (sam_flags[prior] & FLAG_VOWEL)) change(pos, 19, "<VOWEL> L -> <VOWEL> LX"); // Example: ALL
    ...
    if ((sam_flags[Y] & FLAG_DIP_YX)==0 || Y==END) { // VOWELS AND DIPTHONGS ENDING WITH IY SOUND flag set?
  ...
  if ((sam_flags[p] & FLAG_PLOSIVE) && (prior == 32)) { // 'S'
  // RULE:
  //      S P -> S B
  //      S T -> S D
  //      S K -> S G
...
//       <UNSTRESSED VOWEL> D <PAUSE>  -> <UNSTRESSED VOWEL> DX <PAUSE>
// Example: PARTY, TARDY
if (sam_flags[phonemeindex[pos-1]] & FLAG_VOWEL) {
  p = phonemeindex[pos+1];
  if (!p) p = phonemeindex[pos+2];
  if ((sam_flags[p] & FLAG_VOWEL) && !stress[pos+1]) change(pos,30, "Soften T or D following vowel or ER and preceding a pause -> DX");
}
}
}
pos++;
...
void SAM::AdjustLengths() {
  ...
  while((index = phonemeindex[X]) != END) {
    // not punctuation?
    if((sam_flags[index] & FLAG_PUNCT) == 0) {
      ++X;
      continue;
    }
    ...
    while (--X && !(sam_flags[phonemeindex[X]] & FLAG_VOWEL)); // back up while not a vowel
    if (X == 0) break;
    ...
    do {
      ...
      if(!(sam_flags[index] & FLAG_FRICATIVE) || (sam_flags[index] & FLAG_VOICED)) {     //nochmal �berpr�fen
  ...
  while((index = phonemeindex[loopIndex]) != END) {
    ...
    if (sam_flags[index] & FLAG_VOWEL) {
      index = phonemeindex[loopIndex+1];
      if (!(sam_flags[index] & FLAG_CONSONANT)) {
      ...
      unsigned char flag = (index == END) ? 65 : sam_flags[index]; // 65 if end marker
  ...
  } else if((sam_flags[index] & FLAG_NASAL) != 0) { // nasal?
    // RULE: <NASAL> <STOP CONSONANT>
    //       Set punctuation length to 6
    //       Set stop consonant length to 5
    index = phonemeindex[++X];
    if (index != END && (sam_flags[index] & FLAG_STOPCONS)) {
  ...
  } else if((sam_flags[index] & FLAG_STOPCONS)) { // (voiced) stop consonant?
  ...
  } else if ((sam_flags[index] & FLAG_LIQUIC)) { // liquic consonant?
    ...
    if((sam_flags[index] & FLAG_STOPCONS) != 0)
